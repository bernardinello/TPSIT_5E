...
int main(int argc,char **argv)
{
    ...
    getchar();
    scanf("%c",&carattere);
    //invia la stringa e il carattere al server
    write(socketfd,str1,sizeof(str1));
    write(socketfd, &carattere, sizeof(char));
    int count;
    read(socketfd, &count, sizeof(int));

    printf("Il carattere '%c' appare %d volte nella stringa '%s'.\n", carattere, count, str1);
